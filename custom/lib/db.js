...
const Users = {
  ...
  update(id, { full_name, email, role_id, is_active, password }) {
    if (password) {
      const hash = bcrypt.hashSync(password, 10);
      db.prepare(`
...
const Projects = {
  ...
  create({ name, description, workflow_url, n8n_project_id }) {
    const info = db.prepare(`
  ...
  setN8NProjectId(id, n8n_project_id) {
  ...
  update(id, { name, description, workflow_url, is_active }) {
    db.prepare(`
